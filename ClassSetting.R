settingsWindow <- function(dev="FlowCam_GRAYSCALE", zipFile="", prob=0.5, eachRes="1") {
  ...
  tt$env$butDataSelect <- tk2button(tt, text="(Re-)Build a model", image = "update", 
    ...
    dirButton <- tk2button(ttt, text = "SELECT TRAIN/TEST DIRECTORIES", image = "folder", 
                           compound = "left", width = 30, command = function() {
                             trainPath <<- tk_choose.dir(default = getwd())
                             if (is.na(trainPath)) {
                               tkmessageBox(message = "No directory selected!")
                             } else {
                               tkgrid(tklabel(ttt, text=basename(trainPath), font=fontText, foreground = "darkgreen"), 
    ...
    dataVarList <- tk2listbox(ttt, selectmode = "single", activestyle = "dotbox",
                              height = 5, width = 30, autoscroll = "none", background = "white")
    tkgrid(tk2label(ttt, text = "SELECT THE MODEL TO BUILD:"), 
           row = 3, column = 0, pady = c(10,0), sticky = "n", #columnspan=2, 
           padx=c(50,0))
    ...
    ttt$env$epoch <- tkentry(ttt, textvariable=epochValue, width=5, background="white")
    ...
    ttt$env$batch_size <- tkentry(ttt, textvariable=batch_sizeValue, width=5, background="white")
    ...
    ttt$env$img_width <- tkentry(ttt, textvariable=img_widthValue, width=5, background="white")
    ...
    ttt$env$img_height <- tkentry(ttt, textvariable=img_heightValue, width=5, background="white")
    ...
    sapply(pc.title, function(i) {
      radio_button <- ttkradiobutton(ttt, variable = pcValue, 
                                     text = i, value = i)
      tkgrid(radio_button, padx=c(70,50), sticky="w", column=0, columnspan=2)
    })
    
    ValidButton <- tk2button(ttt, text = "BUILD THE MODEL", width = -6, command = function() {
  ...
  tt$env$butSelect <- tk2button(tt, text="Select a model", image = "python", 
    ...
    if (!nchar(zipFile)) {
      ...
      tkgrid(tklabel(tt, text=paste("ZIP file:", basename(zipFile), sep=" "), 
                     font=fontText, foreground = "darkgreen"), sticky="w", row=12, column=0)
      tkgrid(tklabel(tt, text=paste("Model:", sub("\\_.*", "", zipFiles$Name[1]), sep=" "), 
                     font=fontText, foreground = "darkgreen"), sticky="w", row=13, column=0)
      train_res <- read.table(unz(zipFile, zipFiles$Name[grep("history", zipFiles$Name)]), 
                              sep=";", dec=".", header = TRUE)
      tkgrid(tklabel(tt, text=paste("Number of epochs:", train_res$epoch[nrow(train_res)]+1, sep=" "), 
                     font=fontText, foreground = "darkgreen"), sticky="w", row=14, column=0)
      tkgrid(tklabel(tt, text=paste("Training accuracy:", train_res$accuracy[nrow(train_res)], sep=" "), 
                     font=fontText, foreground = "darkgreen"), sticky="w", row=15, column=0)
      ...
      lines(train_res$epoch, train_res$val_accuracy, type="l", col="darkblue", lwd=2)
      grid(lwd=2)
      ...
      lines(train_res$epoch, train_res$val_loss, type="l", col="darkblue", lwd=2)
      grid(lwd=2)
    }
  })
  ...
  tt$env$prob <- tkentry(tt, textvariable=probValue, width=5, background="white")
