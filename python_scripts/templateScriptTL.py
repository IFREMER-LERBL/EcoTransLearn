...
training_data = pd.read_csv(os.path.dirname(trainPath) + "/train.csv", dtype=str, encoding= 'unicode_escape')
test_data = pd.read_csv(os.path.dirname(testPath) + "/test.csv", dtype=str, encoding= 'unicode_escape')
...
RLRP = ReduceLROnPlateau(
        ...
        min_delta=0.0001)
CSV = CSVLogger(
        filename=save_dir + '/' + modelName + '_history.csv',
        separator=';',
        append=True)
...
Checkpoint = ModelCheckpoint(
        #filepath=save_dir + '/' + get_model_name(fold_var),
	filepath=save_dir + '/' + modelName + '_model.h5',
        verbose=1,
        save_freq ='epoch')
...
model.load_weights(save_dir + '/' + modelName + '_model.h5')
...
np.savetxt(save_dir + '/' + modelName + '_predictions.csv', predictions, delimiter=";")
np.savetxt(save_dir + '/' + modelName + '_filenames.csv', test_generator.filenames, delimiter =";", fmt = '% s')
np.savetxt(save_dir + '/' + modelName + '_classes.csv', test_generator.classes, delimiter=";")
np.savetxt(save_dir + '/' + modelName + '_classnames.csv', class_names, delimiter=";", fmt = '% s')

zipObj = ZipFile(save_dir + '/' + modelName + '_' + device + '.zip', 'w')
zipObj.write(save_dir + '/' + modelName + '_classnames.csv', basename(save_dir + '/' + modelName + '_classnames.csv'))
zipObj.write(save_dir + '/' + modelName + '_history.csv', basename(save_dir + '/' + modelName + '_history.csv'))
zipObj.write(save_dir + '/' + modelName + '_predictions.csv', basename(save_dir + '/' + modelName + '_predictions.csv'))
zipObj.write(os.path.dirname(testPath) + '/test.csv', modelName + '_' + basename(os.path.dirname(testPath) + '/test.csv'))
zipObj.write(save_dir + '/' + modelName + '_model.h5', basename(save_dir + '/' + modelName + '_model.h5'))
zipObj.close()