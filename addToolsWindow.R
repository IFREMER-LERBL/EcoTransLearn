addToolsWindow <-
function(opt="Process FlowCam data (collages)") {
  ...
  tkgrid(tk2label(tt, text="What do you want to do?", font=fontTitle), 
         row=1, sticky="w")
  ...
  opt.title <- c("Process FlowCam data (collages)",#"Process CytoSense/Sub data", 
                 ...
                 "Install Python and libraries","Close all figures","Read tutorial", 
                 "About...")
  ...
  onOK <- function() {
    ...
    if (opt=="Generate train and test .CSV files") {
      ...
      dirButton <- tk2button(ttt, text="SELECT TRAIN or TEST DIRECTORIES", 
      ...
      ValidButton <- tk2button(ttt, text="GENERATE CSV", image="csv", 
        ...
        if (is.na(trtePath)) {
          ...
          dattrte <- NULL
          imgf <- list.dirs(trtePath, recursive=FALSE)
          for (i in 1:length(imgf)) {
            datTempImg <- cbind(rep(basename(imgf[i]), 
                                    length(list.files(imgf[i]))), list.files(imgf[i]))
          ...
          write.table(dattrte, paste(trtePath, ".csv", sep=""), 
                      row.names=FALSE, sep=",")
    ...
    if (opt=="Generate metadata file") {
      ...
      dirButton <- tk2button(ttt, text="SELECT DATA DIRECTORY", 
                             image="folder", compound="left", width=40, 
                             command=function() {
      ...
      ValidButton <- tk2button(ttt, text="GENERATE CSV", image="csv", 
        ...
        if (is.na(metaPath)) {
          ...
          metaDirs <- list.dirs(metaPath, recursive=FALSE)
          Sample <- NULL
          Date <- NULL
          Volume <- NULL
          for (i in metaDirs) {
            ...
            smpName <- list.files(i, pattern="_run_summary.txt", 
                                  full.names=TRUE)
            if (length(smpName) > 0) {
              restxt <- readLines(con <- file(smpName, encoding="UTF-8"))
              Date <- c(Date, as.character(str_extract(restxt[grep("Start:", restxt)], 
                                                       "\\d+-\\d+-\\d+ \\d+:\\d+:\\d+")))
              Volume <- c(Volume, as.character(sub(".", ",", 
                                                   str_extract(restxt[grep("Volume Imaged:", restxt)], 
                                                                     "\\d+\\.*\\d*"), 
                                                   fixed=TRUE)))
              close(con)
            }
          ...
          write.table(datmeta, file.path(metaPath, paste(basename(metaPath), 
                                                         "_metadata.csv", sep="")), 
                      row.names=FALSE, sep=";", append=TRUE)
          tkmessageBox(message=paste("Metadata file created and saved in:\n",
                                     file.path(metaPath, paste(basename(metaPath), 
                                                               "_metadata.csv", sep="")), 
                                       sep=""))
          tkdestroy(ttt)
        }
    ...
    if (opt=="Install Python and libraries") {
      msg <- tkmessageBox(message="1. Install 'Anaconda' environment: https://www.anaconda.com/products/individual
                  ...
                  \n3. Install 'Image', 'matplotlib', 'numpy', 'pandas', 'sklearn' and 'tensorflow' libraries")
    }
    ...
    if (opt=="Read tutorial") {
      ...
      manual <- system.file("tutorial", "EcoTransLearn_package_tutorial.pdf", 
